...
export async function POST(request: Request) {
  try {
    ...
    const pixResponse = await createPushinPayPix({
    ...
    store.addEvent(orderId, 'pix_generated', `Cobrança Pix de R$ ${(totalAmountCents / 100).toFixed(2)} gerada na Pushin Pay (#${pixResponse.id}).`);
