...
**Versão:** 0.5  
**Status:** Revisão consolidada com contrato Pushin Pay verificado, segurança de credenciais e webhook operacional

**Status anterior:** Especificação consolidada e alinhada ao branding Notorius, incluindo sistema visual, arquitetura de componentes e referências de direção de arte  
...
### 10.4 Janela comercial, expiração do provedor e resgate tardio de Pix

1. **Janela comercial local:** A interface pode trabalhar com uma janela de **30 minutos** para manter o checkout ativo, orientar o usuário e oferecer geração de uma nova cobrança.
2. **Limitação contratual verificada:** O contrato público atual do endpoint `Criar PIX` documenta `value`, `webhook_url` e `split_rules` no corpo, mas não documenta um parâmetro configurável chamado `expires_at`.
3. **Fonte de verdade:** A aplicação não deve assumir que o QR Code deixou tecnicamente de aceitar pagamento apenas porque a janela local terminou. O estado oficial recebido da Pushin Pay e os webhooks permanecem como fonte de verdade financeira.
4. **Persistência da expiração:**
   - salvar `provider_expires_at` somente se esse campo for retornado pelo provedor em produção;
   - salvar `checkout_expires_at = payment_created_at + 30 minutos` como regra de UX local;
   - não enviar campos não documentados na criação da cobrança.
5. **Reativação Automática de Pix Expirado (`paid_after_expiration`):**
   - O estado `expired` local indica o fim da janela do checkout ativo, mas não inviabiliza o recebimento.
   - Caso um webhook de pagamento aprovado chegue para uma cobrança marcada localmente como `expired`, o backend valida autenticidade, ID externo, valor exato, ausência de processamento prévio e consistência do pedido.
   - Confirmada a validade, a cobrança é atualizada para `paid` com a flag `paid_after_expiration = true`, transicionando o pedido para `payment_status = paid` e disparando o fulfillment automaticamente.
   - A tela pública exibe: **“Pagamento confirmado — Seu pedido já está sendo processado.”**
6. **Invariante de Idempotência Comercial e Cobranças Duplicadas:**
   - Um pedido comercial só inicia o fulfillment exatamente uma vez.
   - Se uma nova cobrança for gerada, todas as tentativas permanecem associadas ao mesmo `order_id`.
   - Se duas cobranças forem pagas, a primeira confirmação válida inicia o fulfillment e o pagamento excedente entra em `duplicate_payment_review`.
...
### 11.0 Segurança de credenciais

O token de produção é um segredo operacional e nunca deve constar:
...
```text
PUSHINPAY_API_TOKEN=68980|zPkRq27cEQqnSb4Ku14ny9pnLtr5EOkku7j33O1Bf38dd0a6
PUSHINPAY_WEBHOOK_URL=https://notorios.com.br/api/webhooks/pushinpay
PUSHINPAY_WEBHOOK_SECRET=Celinawebhook111123
```

Regras:

1. armazenar o token em secret manager ou variável de ambiente protegida;
2. usar credenciais diferentes para produção e sandbox;
3. rotacionar imediatamente qualquer token exposto em conversa, ticket, commit ou screenshot;
4. nunca retornar o token em endpoints administrativos;
5. mascarar qualquer credencial em observabilidade;
6. restringir acesso ao ambiente de produção.

### 11.1 Contrato do endpoint Criar PIX
...
- `value` é obrigatório e enviado em centavos;
- o valor mínimo documentado é de 50 centavos;
- `webhook_url` recebe notificações de pagamento ou estorno;
- `split_rules` é opcional;
- não enviar `expires_at` enquanto esse campo não fizer parte do contrato oficial disponível.

A URL base e o path efetivo devem ser confirmados no API Explorer da conta antes da implementação, pois a página pública carregada não expôs esses valores integralmente no conteúdo textual.

### 11.2 Resposta de criação
...
- `webhook_url`;
- `end_to_end_id`;
- `payer_name`, quando necessário para conciliação;
- `payer_national_registration`, com acesso restrito;
- payload bruto sanitizado;
- timestamps internos.

O `id` deve ser salvo obrigatoriamente, pois identifica a transação e será usado na consulta e conciliação.

### 11.3 Estados externos e mapeamento interno
...
e em outro trecho também referencia `canceled`.

Até testes reais, o adapter deve:

1. mapear os estados conhecidos;
2. armazenar o estado bruto;
3. não falhar ao receber um estado desconhecido;
4. direcionar estados não mapeados para `manual_review`.
...
paid     → paid
expired  → expired
canceled → failed ou canceled, conforme decisão final do domínio
...
```text
https://{dominio-final}/api/webhooks/pushinpay
```

O valor enviado à Pushin Pay precisa ser uma URL HTTPS pública completa. Um nome, apelido ou texto isolado não substitui `webhook_url`.

Fluxo:

1. receber o request;
2. capturar headers e payload;
3. validar o header customizado configurado no painel;
4. persistir o evento bruto sanitizado;
5. deduplicar;
6. localizar pelo `provider_payment_id`;
7. conferir `value`;
8. aplicar a máquina de estados;
9. responder rapidamente;
10. enfileirar fulfillment fora da requisição.

A documentação informa que, quando o webhook falha, a Pushin Pay realiza até três tentativas e permite retomar os envios pelo painel. A documentação também permite configurar um header customizado enviado em todos os webhooks.

### 11.5 Segredo do webhook

Configurar no painel da Pushin Pay um header customizado com valor aleatório e longo.

Exemplo conceitual:

```http
X-Notorius-Webhook-Secret: {SEGREDO}
```

O nome exato do header deve refletir o que o painel permitir configurar. O backend deve comparar o valor recebido usando comparação segura.

Não usar um texto previsível como única camada de segurança. O segredo deve ser rotacionável e armazenado separadamente do token da API.

### 11.6 Idempotência do webhook

Criar uma chave de deduplicação baseada, na ordem de preferência, em:

1. identificador oficial do evento, se existir;
2. combinação de `provider_payment_id + status + end_to_end_id`;
3. hash canônico do payload como fallback.

Regras:

- webhook duplicado de `paid` não cria novo fulfillment;
- transição regressiva não sobrescreve `paid`;
- pagamento já processado retorna sucesso idempotente;
- inconsistência de valor envia para `manual_review`.

### 11.7 Política de retry da integração de criação

Retry automático apenas para falhas em que seja seguro repetir:

- `429`;
- `500`;
- `503`;
- falha de conexão antes do envio;
- indisponibilidade explicitamente transitória.

Em timeout ambíguo após envio do request, consultar a transação quando houver identificador disponível. Sem identificador e sem chave de idempotência documentada, marcar `payment_creation_unknown` e evitar criar cobranças repetidas automaticamente.

### 11.8 Aviso obrigatório no checkout

Antes da finalização do pagamento, incluir aviso visível:

> A Pushin Pay atua exclusivamente como processadora de pagamentos e não possui responsabilidade pela entrega, suporte, conteúdo, qualidade ou cumprimento das obrigações relacionadas aos produtos ou serviços oferecidos pela Notorius.

Aplicações:

- abaixo do resumo de pagamento;
- termos de compra;
- comunicações automáticas relacionadas à cobrança.

O texto deve ter legibilidade real e não ficar oculto em tooltip ou modal opcional.

### 11.9 Sandbox

Para utilizar sandbox, a documentação orienta:

1. possuir cadastro de produção;
2. solicitar ao suporte a liberação do ambiente sandbox;
3. manter token, URL e dados de sandbox separados dos de produção.

Não testar o fluxo inicial com o token de produção em ambientes de preview.

### 11.10 Dados mínimos a persistir
...
- `webhook_url`;
- `end_to_end_id`;
- `payer_name`, quando recebido;
- `payer_national_registration`, com proteção;
- `checkout_expires_at`;
- `provider_expires_at`, apenas quando retornado;
- `paid_after_expiration`;
- `raw_create_response`;
- `created_at`;
- `paid_at`.

### 11.11 Pendências para teste de integração

Antes de considerar a integração concluída, obter exemplos reais de:

- payload de webhook de pagamento;
- payload de webhook de expiração;
- payload de webhook de estorno;
- nome e formato do header customizado;
- endpoint de consulta por ID;
- Base URL de produção;
- Base URL de sandbox;
- comportamento real de expiração;
- resposta de cobrança cancelada;
- limites por conta;
...
- Pushin Pay — documentação Theneo, Criar PIX: https://app.theneo.io/pushinpay/pix/pix/criar-pix
